...
export default function ChampionshipTab({
  ...
  const { selectedContractId, setSelectedContractId, backingSide } = useTerminalUI();
    ...
    <div className="h-full flex flex-col md:flex-row gap-4 min-h-0 font-mono text-body-sm text-slate-400">
      ...
      <div className="w-full md:w-44 shrink-0 flex flex-row md:flex-col gap-1 border-b md:border-b-0 md:border-r border-white/5 pb-2 md:pb-0 pr-0 md:pr-3">
        <button
          ...
          className={`px-3 py-2 text-left font-bold rounded text-body-sm cursor-pointer transition-colors block shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-terminal-blue-light/60 ${selectedChampionship === "DRIVERS" ? "bg-terminal-blue/15 border-l-2 border-terminal-blue text-white" : "bg-white/3 hover:bg-white/5 text-slate-400"}`}
        >
          Drivers Championship
        </button>
        <button
          onClick={() => handleChampionshipChange("CONSTRUCTORS")}
          className={`px-3 py-2 text-left font-bold rounded text-body-sm cursor-pointer transition-colors block shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-terminal-blue-light/60 ${selectedChampionship === "CONSTRUCTORS" ? "bg-terminal-blue/15 border-l-2 border-terminal-blue text-white" : "bg-white/3 hover:bg-white/5 text-slate-400"}`}
        >
      ...
      <div className="flex-1 panel-raised rounded p-3.5 flex flex-col min-w-0 min-h-0">
        ...
        <div className="flex-1 overflow-y-auto pr-1 min-h-0">
          <table className="w-full text-left font-data text-body-sm">
            <thead>
