...
export default function MarketTab({ contracts, positions, isFocusMode }: MarketTabProps) {
  const { selectedContractId, setSelectedContractId } = useTerminalUI();
  ...
  return (
    <div className="h-full flex flex-col md:flex-row gap-3.5 min-h-0">
      {/* Category filter */}
      <div className="w-full md:w-40 shrink-0 flex flex-row md:flex-col gap-1 md:pr-1">
        {CATEGORIES.map((cat) => (
          <button
            key={cat.id}
            onClick={() => setSelectedCategory(cat.id)}
            className={`px-3 py-2 text-left font-bold rounded text-body-sm cursor-pointer transition-colors block shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-terminal-blue-light/60 ${selectedCategory === cat.id ? "bg-terminal-blue/15 border-l-2 border-terminal-blue text-white" : "bg-white/3 hover:bg-white/5 text-slate-400"}`}
          >
            {cat.label}
          </button>
        ))}
