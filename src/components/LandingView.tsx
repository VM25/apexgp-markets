...
import { Cpu, Landmark, Activity, GraduationCap, Compass } from "lucide-react";

interface LandingViewProps {
  /** withOrientation: whether to auto-expand the desk-orientation checklist on entry. */
  onEnterTerminal: (withOrientation: boolean) => void;
}

const ORIENT_PREF_KEY = "apexgp_orientation_optin";

export default function LandingView({ onEnterTerminal }: LandingViewProps) {
  const [isCollapsing, setIsCollapsing] = useState<boolean>(false);
  // Default ON for first-time users; persisted so returning users keep their choice.
  const [startWithOrientation, setStartWithOrientation] = useState<boolean>(true);

  useEffect(() => {
    try {
      const stored = localStorage.getItem(ORIENT_PREF_KEY);
      // Hydration-safe preference read: must run post-mount, off the render path.
      // eslint-disable-next-line react-hooks/set-state-in-effect
      if (stored === "false") setStartWithOrientation(false);
    } catch {
      /* ignore */
    }
  }, []);
  ...
  const handleEnterClick = () => {
    try {
      localStorage.setItem(ORIENT_PREF_KEY, startWithOrientation ? "true" : "false");
    } catch {
      /* ignore */
    }
    setIsCollapsing(true);
    setTimeout(() => {
      onEnterTerminal(startWithOrientation);
    }, 350);
  };

  return (
    <div className={`relative min-h-dvh bg-carbon-black flex flex-col items-center justify-center px-4 py-10 sm:p-6 overflow-x-hidden select-none transition-all duration-350 font-mono text-micro text-slate-450 ${isCollapsing ? "opacity-0 scale-95 pointer-events-none" : "opacity-100 scale-100"}`}>

      {/* Background Quantitative Telemetry Feed Stream (Academic Atmosphere) */}
      <div className="absolute inset-0 opacity-[0.02] pointer-events-none text-micro leading-relaxed p-8 overflow-hidden hidden sm:block">
      ...
      <div className="relative z-10 max-w-5xl w-full bg-carbon-dark/90 border border-white/5 rounded p-5 sm:p-8 md:p-12 shadow-2xl flex flex-col gap-8 md:gap-10">
        ...
        <div className="border-b border-white/5 pb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-body-sm font-light leading-relaxed max-w-2xl font-sans mt-1">
          ...
          <div className="flex items-start gap-2.5 bg-white/3 border border-white/5 rounded p-2.5 w-full sm:w-auto sm:max-w-xs shrink-0 select-text">
            <GraduationCap className="w-5 h-5 text-terminal-blue shrink-0 mt-0.5" />
            <div className="space-y-0.5 leading-tight">
              <span className="text-white font-bold block text-body-sm">Vatsal Maniar</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-sans">
          ...
          <div className="space-y-2">
            ...
            <p className="text-slate-400 text-body-sm leading-relaxed font-light">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center pt-2">
          ...
          <div className="flex flex-col items-stretch sm:items-end gap-2.5 w-full sm:w-auto">
            <button
              ...
              className="w-full sm:w-auto min-h-[44px] px-8 py-3 bg-terminal-blue hover:bg-terminal-blue-light text-white font-mono font-bold text-body-sm tracking-widest uppercase rounded cursor-pointer transition-all duration-200 active:scale-[0.98] shadow-lg shadow-terminal-blue/15 text-center outline-none focus-visible:ring-2 focus-visible:ring-terminal-blue-light/60 focus-visible:ring-offset-2 focus-visible:ring-offset-carbon-dark"
            >
              Enter Market Terminal ➔
            </button>

            {/* First-run choice: start with desk orientation (default ON). */}
            <button
              type="button"
              role="checkbox"
              aria-checked={startWithOrientation}
              onClick={() => setStartWithOrientation((v) => !v)}
              className="w-full sm:w-auto min-h-[36px] flex items-center gap-2 px-2 py-1.5 rounded text-micro font-sans text-slate-400 hover:text-slate-200 cursor-pointer transition-colors outline-none focus-visible:ring-2 focus-visible:ring-terminal-blue-light/60"
            >
              <span
                className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors ${startWithOrientation ? "bg-terminal-blue border-terminal-blue" : "border-white/20 bg-carbon-black"}`}
                aria-hidden="true"
              >
                {startWithOrientation && <Compass className="w-2.5 h-2.5 text-white" />}
              </span>
              <span className="font-bold uppercase tracking-wider">Start with desk orientation</span>
            </button>
          </div>
        </div>
      ...
      <div className="static mt-8 sm:absolute sm:bottom-4 sm:mt-0 text-center z-10 text-micro text-slate-600 font-mono tracking-wider font-bold px-4">
