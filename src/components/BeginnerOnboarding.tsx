  ...
  const beginnerSteps = [
    {
      ...
      example: (
        <div className="space-y-2">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7px] tracking-wider">CANONICAL WORKED PAYOUT EXAMPLE (BUY YES):</span>
          <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 font-mono text-[9px] text-slate-350 bg-carbon-black/60 p-2.5 rounded border border-white/5">
            ...
            <div className="space-y-0.5 mt-1 border-r border-white/5 pr-2">
              <span className="text-terminal-green-light font-bold block text-[8px] uppercase">▲ EVENT SUCCEEDS (TRUE)</span>
            ...
            <div className="space-y-0.5 mt-1 pl-2">
              <span className="text-terminal-red font-bold block text-[8px] uppercase">▼ EVENT FAILS (FALSE)</span>
    ...
    },
    {
      ...
      example: (
        <div className="space-y-1.5 text-[8.5px] font-mono">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7.5px] tracking-wider mb-1">TERMINOLOGY FOCUS:</span>
    ...
    },
    {
      ...
      example: (
        <div className="space-y-1.5 text-[8.5px] font-sans font-light">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7.5px] tracking-wider mb-1">PROGRESSIVE TIMELINE:</span>
    ...
    },
    {
      ...
      example: (
        <div className="space-y-1.5 text-[8.5px] font-mono">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7.5px] tracking-wider mb-1">SETTLEMENT FLOW:</span>
  ...
  const advancedSteps = [
    {
      ...
      example: (
        <div className="space-y-2">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7px] tracking-wider">RISK LIMIT THRESHOLDS:</span>
          <div className="grid grid-cols-2 gap-2 text-[8.5px] font-mono">
            <div className="bg-carbon-black/80 p-2 border border-white/5 rounded">
              <span className="text-slate-500 block text-[6.5px] uppercase font-bold">SINGLE CONTRACT</span>
              <span className="text-terminal-red font-bold block mt-0.5">15% Max Exposure</span>
              <p className="text-[6.5px] text-slate-400 mt-1 leading-normal">Max capital allocation on one contract outcome.</p>
            </div>
            <div className="bg-carbon-black/80 p-2 border border-white/5 rounded">
              <span className="text-slate-500 block text-[6.5px] uppercase font-bold">PORTFOLIO AT RISK</span>
              <span className="text-terminal-red font-bold block mt-0.5">30% Max at Risk</span>
              <p className="text-[6.5px] text-slate-400 mt-1 leading-normal">Max cumulative risk across all open contracts.</p>
            </div>
          </div>
        </div>
      )
    },
    {
      ...
      example: (
        <div className="space-y-1.5 text-[8.5px] font-sans font-light">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7.5px] tracking-wider mb-1">LONG-TERM RISK:</span>
    ...
    },
    {
      ...
      example: (
        <div className="space-y-1 text-[8.5px] font-mono">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7.5px] tracking-wider mb-1">TELEMETRY INPUTS TO PRICE:</span>
          <div className="bg-carbon-black/60 p-2 rounded border border-white/5 space-y-1 text-[8px]">
  ...
  const quantSteps = [
    {
      ...
      example: (
        <div className="space-y-2">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7px] tracking-wider">GREEKS EXPOSURE DICTIONARY:</span>
          <div className="grid grid-cols-3 gap-1.5 text-[8px] leading-snug font-mono">
            <div className="bg-carbon-black p-1.5 rounded border border-white/5 text-center">
              <span className="text-terminal-blue block font-extrabold text-[9px]">Δ (Delta)</span>
              <span className="text-slate-400 block mt-0.5 text-[7px] leading-none">Sensitivity to odds shift</span>
            </div>
            <div className="bg-carbon-black p-1.5 rounded border border-white/5 text-center">
              <span className="text-terminal-green-light block font-extrabold text-[9px]">Θ (Theta)</span>
              <span className="text-slate-400 block mt-0.5 text-[7px] leading-none">Uncertainty decay per lap</span>
            </div>
            <div className="bg-carbon-black p-1.5 rounded border border-white/5 text-center">
              <span className="text-terminal-gold block font-extrabold text-[9px]">IV Index</span>
              <span className="text-slate-400 block mt-0.5 text-[7px] leading-none">Implied track uncertainty</span>
            </div>
          </div>
        </div>
      )
    },
    {
      ...
      example: (
        <div className="space-y-2">
          <span className="text-terminal-blue-light font-bold block uppercase text-[7px] tracking-wider">EXPONENTIAL CONVERGENCE CONSTRUCT:</span>
          <code className="text-terminal-blue-light block text-[8.5px] py-1 px-2 bg-carbon-black border border-white/5 rounded font-bold font-mono">
            {"P(θ | Telemetry) = (1 - w) * Prior + w * RealizedOutcome"}
          </code>
          <span className="text-slate-400 text-[8px] block font-sans font-light leading-relaxed">
      ...
      <div className="relative max-w-4xl w-full bg-carbon-dark border border-terminal-blue/20 rounded-lg p-6 shadow-[0_0_40px_rgba(0,122,204,0.15)] flex flex-col justify-between h-[510px] min-h-[510px] max-h-[510px] overflow-hidden">
        ...
        <div className="flex justify-between items-center border-b border-white/5 pb-2.5 shrink-0">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={handleSkip}
              className="text-[8.5px] font-bold text-slate-500 hover:text-slate-300 uppercase cursor-pointer transition-colors border border-white/5 px-2 py-0.5 rounded bg-white/2 hover:bg-white/5"
            >
              Skip Tutorial
            </button>
            <button
        ...
        <div className="flex border-b border-white/5 shrink-0 select-none bg-carbon-black/20 mt-2 rounded overflow-hidden">
          {[
            ...
            <button
              ...
              className={`flex-1 py-2 font-bold text-[9px] flex items-center justify-center gap-1.5 transition-colors cursor-pointer border-b-2 ${activeTab === t.id ? "bg-carbon-light text-white border-terminal-blue" : "border-transparent text-slate-500 hover:text-slate-400"}`}
            >
              {t.icon}
              {t.label}
            </button>
          ...
          {activeTab !== "GLOSSARY" ? (
            <>
              ...
              <div className="w-48 shrink-0 border-r border-white/5 pr-3 space-y-1 select-none overflow-y-auto">
                {steps.map((s, idx) => (
                  <button
                    ...
                    className={`w-full p-2.5 text-left font-bold rounded text-[8.5px] leading-tight block cursor-pointer transition-all border-l-2 ${activeStep === idx ? "bg-white/5 text-white border-l-terminal-blue" : "border-l-transparent text-slate-500 hover:text-slate-400"}`}
                  >
                    {s.title.substring(3)}
                  </button>
                ))}
              ...
              <div className="flex-1 overflow-y-auto space-y-3 pr-1 select-text">
                <div className="flex justify-between items-center uppercase font-bold text-[7px] text-slate-550 tracking-wider">
                  <span>ACTIVE DESK: {activeTab} // SEGMENT</span>
                  <span className="font-data">STEP {activeStep + 1} OF {steps.length}</span>
                </div>
                
                <h4 className="text-white font-extrabold text-[11px] uppercase tracking-wider">
                  {currentStep?.title}
                </h4>
                
                <p className="text-slate-300 leading-relaxed font-sans font-light text-[9.5px]">
                  {currentStep?.content}
                </p>
              ...
              <div className="w-64 shrink-0 border-r border-white/5 pr-3 space-y-1 select-none overflow-y-auto">
                {glossaryTerms.map((g, idx) => (
                  <button
                    ...
                    className={`w-full p-2 text-left font-bold rounded text-[8.5px] leading-tight flex justify-between items-center cursor-pointer transition-all border-l-2 ${selectedGlossaryTerm === idx ? "bg-white/5 text-white border-l-terminal-blue" : "border-l-transparent text-slate-500 hover:text-slate-400"}`}
                  >
                    <span>{g.term}</span>
                    <span className="text-[6.5px] text-slate-650 font-bold uppercase">{g.category}</span>
                  </button>
                ))}
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-3.5 pr-1 font-mono">
                ...
                <div className="bg-carbon-black/40 border border-white/5 rounded p-3 space-y-3">
                  <div className="flex justify-between items-baseline">
                    <h4 className="text-terminal-blue-light font-extrabold text-[12px] uppercase tracking-wider">
                      {glossaryTerms[selectedGlossaryTerm].term}
                    </h4>
                    <span className="px-2 py-0.5 bg-terminal-blue/10 border border-terminal-blue/20 text-terminal-blue-light rounded font-bold text-[7.5px] uppercase">
                  ...
                  <div className="space-y-2 text-[9.5px]">
                    <div className="text-slate-200 font-sans font-light leading-relaxed">
                      <span className="text-[7.5px] text-slate-550 font-bold uppercase block tracking-wider font-mono">DEFINITION:</span>
                      {glossaryTerms[selectedGlossaryTerm].definition}
                    </div>

                    <div className="border-t border-white/5 pt-2 grid grid-cols-2 gap-2 text-[8.5px]">
                      <div>
                        <span className="text-[7.5px] text-slate-500 font-bold uppercase block tracking-wider mb-0.5">RACING CONTEXT:</span>
                        <span className="text-slate-350">{glossaryTerms[selectedGlossaryTerm].racing}</span>
                      </div>
                      <div>
                        <span className="text-[7.5px] text-slate-500 font-bold uppercase block tracking-wider mb-0.5">QUANT LOGIC:</span>
                ...
                <div className="bg-terminal-blue/5 border border-terminal-blue/15 rounded p-2.5 text-[8.5px] text-slate-400 flex items-start gap-2 font-sans font-light">
        ...
        <div className="flex justify-between items-center pt-3 border-t border-white/5 shrink-0 h-10 min-h-[40px]">
          <span className="text-[7.5px] text-slate-550 font-bold tracking-wider uppercase select-none">
          ...
          <div className="flex gap-2">
            {(activeStep > 0 || activeTab !== "BEGINNER") && (
              <button
                type="button"
                onClick={handlePrev}
                className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-white rounded font-bold text-[9px] uppercase tracking-wider flex items-center gap-1 cursor-pointer transition-all border border-white/10"
              >
            ...
            <button
              type="button"
              onClick={handleNext}
              className="px-3 py-1.5 bg-terminal-blue hover:bg-terminal-blue-light text-white rounded font-bold text-[9px] uppercase tracking-wider flex items-center gap-1 cursor-pointer transition-all border border-terminal-blue-light"
            >
              {activeTab === "GLOSSARY" ? (
